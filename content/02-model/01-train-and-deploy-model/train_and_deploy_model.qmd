...
if (interactive()) {
  # If the code is being run interactively only train a few days so that it runs
  # quicker.
  n_days_to_train <- 10
} else {
  # When the code is not being run interactivly train on more data. This will
  # take longer but result in a better model.
  n_days_to_train <- 30 * 6
}
...
board %>%
 vetiver_pin_write(
   v,
   title = "Bike Predict (R) - Model - Random Forest Model Pin",
 )
```
...
# Get the content
content <-client %>%
  connectapi::content_item("a8b9ad3e-f847-4a6c-aadb-01684873b8e5")
...
```{r inspect_history_of_model_pins}
